...
void Vbranch_tb::eval_end_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+eval_end_step Vbranch_tb::eval_end_step\n"); );
#ifdef VM_TRACE
    // Tracing
...
std::unique_ptr<VerilatedTraceConfig> Vbranch_tb::traceConfig() const {
...
void Vbranch_tb___024root__trace_decl_types(VerilatedVcd* tracep);

void Vbranch_tb___024root__trace_init_top(Vbranch_tb___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vbranch_tb___024root__trace_decl_types(tracep);
    Vbranch_tb___024root__trace_init_top(vlSelf, tracep);
    tracep->popPrefix();
}

VL_ATTR_COLD void Vbranch_tb___024root__trace_register(Vbranch_tb___024root* vlSelf, VerilatedVcd* tracep);

VL_ATTR_COLD void Vbranch_tb::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    (void)levels; (void)options;
    VerilatedVcdC* const stfp = dynamic_cast<VerilatedVcdC*>(tfp);
    if (VL_UNLIKELY(!stfp)) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vbranch_tb::trace()' called on non-VerilatedVcdC object;"
            " use --trace-fst with VerilatedFst object, and --trace with VerilatedVcd object");
    }
    stfp->spTrace()->addModel(this);
    stfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vbranch_tb___024root__trace_register(&(vlSymsp->TOP), stfp->spTrace());
}
